...
public class Arbol<T extends Comparable<T>> {
    ...
    private NodoBST<T> eliminar_privado(NodoBST<T> actual, T dato) {
        if (actual.dato.compareTo(dato) == 0) { // el dato corresponde
            ...
            if (actual.izq == null && actual.der == null) {
                ...
                NodoBST<T> x = actual.der.buscarSucesor();
                // reamplazar el dato con el sucesor
                x = eliminar_privado(actual, dato);
                actual.dato = x.dato;
                actual = eliminar_privado(actual.der, x.dato);
                return actual;
            }
    ...
    public static void main(String[] args) {
        ...
        nuevo.imprimir();
        // System.out.println(nuevo.longitudCaminoInterno());
